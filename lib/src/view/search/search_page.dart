...
class _SearchPageState extends State<SearchPage> with SingleTickerProviderStateMixin{
  ...
  void initState() {
    ...
    // Listen for tab changes and reset the scroll-to-top visibility
    _tabController.addListener(() {
      if (_tabController.indexIsChanging) {
        // Reset the scroll-to-top visibility when switching tabs
        searchCon.isMovieScrollToTopVisible(false);
        searchCon.isSeriesScrollToTopVisible(false);
      }
    });
  }
    ...
    return Scaffold(
      ...
      body: Stack(
        children: [
                      ...
                      : NotificationListener<ScrollNotification>(
                        onNotification: (scrollNotification) {                            
                          ...
                          if (seriesScrollController.position.pixels  > 2000) {
                            searchCon.isSeriesScrollToTopVisible(true);
          ...
          Obx(() {
            return Visibility(
              visible: searchCon.isMovieScrollToTopVisible.value && _tabController.index == 0,
              child: Positioned(
                bottom: 20,
                right: 20,
                child: FloatingActionButton(
                  ...
                  onPressed: () {
                    movieScrollController.animateTo(
                      0,
          ...
          Obx(() {
            return Visibility(
              visible: searchCon.isSeriesScrollToTopVisible.value && _tabController.index == 1,
              child: Positioned(
                bottom: 20,
                right: 20,
                child: FloatingActionButton(
                  backgroundColor: const Color(0xffecc877),
                  onPressed: () {
                    seriesScrollController.animateTo(
                      0,
                      duration: const Duration(milliseconds: 800),
                      curve: Curves.easeOut,
                    );
